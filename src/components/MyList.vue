  ...
  <div id="myList" class="myList">
    <ul>
      <li v-for="item in list" :key="item.product_id">
        ...
        <router-link :to="{ path: '/details', query: {productID:item.product_id} }" tag="a">
      ...
      <li v-show="isMore && list.length>=1" id="more">
        <router-link :to="{ path: '/lists', query: {categoryID:categoryID} }" tag="a">
          浏览更多
...
export default {
    ...
    categoryID: function() {
      ...
      if (this.list != "") {
        for (let i = 0; i < this.list.length; i++) {
          ...
          if (!categoryID.includes(id)) { //用来判断一个数组是否包含一个指定的值，如果是返回 true，否则false。
            categoryID.push(id);
          }
        }
      }
  ...
  methods: {
    deleteCollect(product_id) {
        this.$store.commit("collect/deleteCollect",product_id);
        // 提示删除成功信息
