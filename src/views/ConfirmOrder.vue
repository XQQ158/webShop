    ...
    <div class="content">
      ...
      <div class="section-goods">
        ...
        <div class="goods-list">
          <ul>
            <li v-for="item in confirmLists" :key="item.product_id">
              <img :src="item.product_picture" />
              <span class="pro-name">{{item.product_name}}</span>
              <span class="pro-price">{{item.product_price}}元 x {{item.product_num}}</span>
              <span class="pro-status"></span>
              <span class="pro-total">{{item.product_price * item.product_num}}元</span>
            </li>
          </ul>
        </div>
      </div>
        ...
        <div class="money-box">
          <ul>
            <li>
              ...
              <span class="value">{{totalNum.num}}件</span>
            </li>
            <li>
              <span class="title">商品总价：</span>
              <span class="value">{{totalNum.total}}元</span>
            </li>
            <li>
            ...
            <li class="total">
              ...
              <span class="value">
                <span class="total-price">{{totalNum.total}}</span>元
              </span>
            </li>
          </ul>
        </div>
...
export default {
      ...
      address: [
        {
          ...
          name: "小明",
          phone: "13580018623",
          address: "广东 广州市 海珠区 新港西路135号"
        },
        {
          id: 2,
          name: "小红",
          phone: "13580018623",
          address: "广东 广州市 海珠区 新港西路105号"
        }
      ],
      confirmLists:[],
    };
  ...
  created() {
    this.confirmLists = this.$store.state.cart.shoppingCart;
  },
  computed:{
    totalNum() {
      return this.$store.state.cart.checkGoods
    },
  },
  methods: {
    //点击结算
    addOrder() {
        ...
        let order = {
            ...
            goodsList:this.confirmLists,
            dateTime:new Date(),
            checkList:this.$store.state.cart.checkGoods,
            // type:1 //未支付
        }
        // 调用vuex存入订单数据状态
        this.$store.commit("order/add",order);
        //清空购物车
        this.$store.commit("cart/empytCart");
        this.$store.commit("cart/empytGoods");
        // 提示结算结果
