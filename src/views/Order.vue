      ...
      <div class="content" v-for="(item,index) in orders" :key="index">
        <ul>
          ...
          <li class="order-info">
            ...
            <div class="order-time">订单时间: {{item.dateTime | dateFormat}}</div>
          </li>
          <li class="header">
          ...
          <!-- 订单列表 -->
          <li class="product-list" v-for="(value,index) in item.goodsList" :key="index">
            <div class="pro-img">
              <router-link :to="{ path: '/details', query: {productID:value.product_id} }">
                <img :src="value.product_picture" />
              </router-link>
            </div>
            <div class="pro-name">
              <router-link
                :to="{ path: '/details', query: {productID:value.product_id} }"
              >{{value.product_name}}</router-link>
            </div>
            <div class="pro-price">{{value.product_price}}元</div>
            <div class="pro-num">{{value.product_num}}</div>
            <div class="pro-total pro-total-in">{{value.product_price*value.product_num}}元</div>
          </li>
        </ul>
        <div class="order-bar">
          <div class="order-bar-left">
            <span class="order-total">
              共
              <span class="order-total-num">{{item.checkList.num}}</span> 件商品
            </span>
          </div>
          <div class="order-bar-right">
            <span>
              <span class="total-price-title">合计：</span>
              <span class="total-price">{{item.checkList.total}}元</span>
            </span>
          </div>
          <!-- 订单列表END -->
        </div>
...
export default {
    ...
    created() {
      // 获取订单数据
      this.orders = this.$store.state.order.orderList;
    },
